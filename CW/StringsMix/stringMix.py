...
# letters and digits - more precisely sorted by codepoint); the different groups will be separated by '/'. See
...
def mix(s1, s2):
    ...
    for ch in "abcdefghijklmnopqrstuvwxyz":
        a, b = s1.count(ch), s2.count(ch)
        if max(a, b) > 1:
            which = "1" if a > b else "2" if b > a else "="
            data[ch] = (-max(a, b), which + ":" + ch * max(a, b))
    return "/".join(data[ch][1] for ch in sorted(data, key=lambda x: data[x]))